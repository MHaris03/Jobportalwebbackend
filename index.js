  ...
  try {
    ...
    app.get('/user-applied-jobs', authenticateToken, async (req, res) => {
      ...
      try {
        ...
        const jobsCursor = await jobsCollections
          .find({ _id: { $in: jobIds } })
          .toArray();

        const jobsWithAppliedDate = jobIds.map(jobId => {
          const job = jobsCursor.find(j => j._id.toString() === jobId.toString());
          const application = applications.find(app => app.jobId.toString() === jobId.toString());
          return {
            ...job,
